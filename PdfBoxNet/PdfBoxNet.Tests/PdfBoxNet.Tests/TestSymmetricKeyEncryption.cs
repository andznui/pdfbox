    ...
    public class TestSymmetricKeyEncryption
    {
                ...
                AccessPermission permission)
        {
            try
            {
                ...
                List<java.awt.image.BufferedImage> srcImgTab = new List<java.awt.image.BufferedImage>();
                List<byte[]> srcContentStreamTab = new List<byte[]>();
                for (int i = 0; i < numSrcPages; ++i)
                {
                    srcImgTab.Add(pdfRenderer.renderImage(i));
                    java.io.InputStream unfilteredStream = document.getPage(i).getContents();
                    byte[] streamBytes = org.apache.pdfbox.io.IOUtils.toByteArray(unfilteredStream);
                    unfilteredStream.close();
                    srcContentStreamTab.Add(streamBytes);
                }
                        ...
                        prefix, permission, userpassword, ownerpassword))
                {
                    ...
                    {
                        // compare rendering
                        java.awt.image.BufferedImage bim = pdfRenderer.renderImage(i);
                        ValidateXImage.checkIdent(bim, srcImgTab[i]);

                        // compare content streams
                        java.io.InputStream unfilteredStream = encryptedDoc.getPage(i).getContents();
                        byte[] bytes = org.apache.pdfbox.io.IOUtils.toByteArray(unfilteredStream);
                        unfilteredStream.close();
        ...
        private java.io.File extractEmbeddedFile(RandomAccessRead pdfSource, string name)
        {
            ...
            Assert.Equal(1, embeddedFileNames.size());

            // Get the first entry from the Java Map
            var entrySet = embeddedFileNames.entrySet();
            var iterator = entrySet.iterator();
            var entry = (java.util.Map.Entry)iterator.next();
            var complexFileSpec = (org.apache.pdfbox.pdmodel.common.filespecification.PDComplexFileSpecification)entry.getValue();
            var embeddedFile = complexFileSpec.getEmbeddedFile();

            java.io.File resultFile = new java.io.File(TESTRESULTSDIR, name);
            using (var netOutputStream = File.Create(resultFile.getAbsolutePath()))
            {
                java.io.InputStream javaInputStream = embeddedFile.createInputStream();
                byte[] buffer = org.apache.pdfbox.io.IOUtils.toByteArray(javaInputStream);
                javaInputStream.close();
                netOutputStream.Write(buffer, 0, buffer.Length);
            }
